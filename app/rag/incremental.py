...
class IncrementalIngestService:
    ...
    def __init__(self) -> None:
        ...
        self._kb_dir = Path(settings.watch_path).resolve()
        self._debounce_seconds = settings.watch_debounce_seconds
        self._hash_store = HashStore(Path(settings.hash_store_path))
        self._embedder = get_embedding_backend(settings.embedding_model)
    ...
    def _handle_upsert(self, path: Path, event_type: str) -> None:
        ...
        if stored and stored.get("content_hash") == content_hash:
            logger.info("Hash unchanged, skipping rebuild: event=%s source=%s doc_id=%s", event_type, source_path, doc_id)
            return

        previous_chunks = delete_document(doc_id)
