...
| v1.2.1 | 2026-08-25 | D-5 修复冲刺执行：P0 综述缺陷根治 + 自检合并策略修正（`6389756`）；D-2 前端五项修复完成——单徽章+tooltip 披露、SSE info 可见化、自检 reasons 可点击、.bib 下载（`6850d99`）；FR-3 过滤器 UI 落地（`359f54b`）；CI 依赖 pin 组验证通过（chromadb 0.6.3+langchain-core 0.3.86），剩余引导阻塞为一次性 ONNX 模型联网下载 |
...
| FR-7 综述工作台 | ✅ MVP 完成（后端） | `f25d285` | `POST /frontend/review-workbench`：review.v1 结构化负载 + 可点击引用 + 无 runtime 时确定性抽取降级；前端专用渲染面板未做（当前走通用 JSON 渲染）。**⚠️ 辩论发现 P0 缺陷，见第 14 节 D-1** → **✅ D-1 缺陷已修复**（`6389756`：假信号摘除、runtime 注入、task_type 实名；诚实降级有测试锁定） |
...
### D-1 辩论发现的 P0 缺陷（综述工作台，必修）→ ✅ 已修复（v1.2.1，`6389756`）
