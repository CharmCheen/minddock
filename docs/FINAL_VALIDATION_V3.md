...
| TC1 | structured reference | Table 1 chunk 看不到，后续跨源污染 | Phase 2B-3 / 2B-4 | top citation 命中 Table 1 附近，final citations 保持 Milvus PDF | pass |
...
| N1 | normal factual | 低位 citation 混入 local docs | Phase 2B-4 | citations 保持 Milvus PDF | pass |
...
Phase 2B-3 解决候选可见性问题，Phase 2B-4 减少低位 source 污染。
...
- citations 主要来自 Milvus PDF。
- 不再混入 `api_usage.md` / `example.md` 作为主要 citations。

结论：

Phase 2B-4 的 source consistency cap 生效。
